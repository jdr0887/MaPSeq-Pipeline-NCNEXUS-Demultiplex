...
public class NCNEXUSCASAVAServiceImplTest {
    ...
    public void testUpload() {
        ...
        NCNEXUSDemultiplexService casavaService = service.getPort(NCNEXUSDemultiplexService.class);

        Client cl = ClientProxy.getClient(casavaService);
        HTTPConduit httpConduit = (HTTPConduit) cl.getConduit();
        httpConduit.getClient().setReceiveTimeout(5 * 60 * 1000L);

        Binding binding = ((BindingProvider) service.getPort(portQName, NCNEXUSDemultiplexService.class)).getBinding();
